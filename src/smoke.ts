...
const dashboard = await admin.request<{ counts: { openChats: number } }>("GET", "/admin/dashboard");
assert(typeof dashboard.counts.openChats === "number", "admin dashboard should return counts");
const agents = await admin.request<{ items: unknown[] }>("GET", "/admin/agents");
assert(agents.items.length > 0, "admin agents should list agents");
const customers = await admin.request<{ items: unknown[] }>("GET", "/admin/customers");
